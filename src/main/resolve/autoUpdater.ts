...
export async function checkUpdate(): Promise<IAppVersion | undefined> {
  ...
  const githubUrl =
    'https://github.com/nett-party-org/nett-party/releases/latest/download/latest.yml'
...
export async function downloadAndInstallUpdate(version: string): Promise<void> {
  ...
  const githubBase = `https://github.com/nett-party-org/nett-party/releases/download/v${version}/`
  const fileMap = {
    'win32-x64': `nett-party-windows-${version}-x64-setup.exe`,
    'win32-ia32': `nett-party-windows-${version}-ia32-setup.exe`,
    'win32-arm64': `nett-party-windows-${version}-arm64-setup.exe`,
    'darwin-x64': `nett-party-macos-${version}-x64.pkg`,
    'darwin-arm64': `nett-party-macos-${version}-arm64.pkg`
  }
