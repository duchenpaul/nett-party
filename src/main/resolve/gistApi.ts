...
async function createGist(token: string, content: string): Promise<void> {
  ...
  await chromeRequest.post(
    ...
    {
      description: 'Auto Synced nett-party Runtime Config',
      public: false,
      files: { 'nett-party.yaml': { content } }
    },
    {
      headers: {
        Accept: 'application/vnd.github+json',
...
export async function getGistUrl(): Promise<string> {
  ...
  const gist = gists.find((gist) => gist.description === 'Auto Synced nett-party Runtime Config')
  if (gist) {
